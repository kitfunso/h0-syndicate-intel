 ...
 * into Aurora and embeds every narrative chunk via Bedrock Titan. Computes the
 * GBP-normalised value from native value x fx_rate. The 2023 report is created once
 * per syndicate; comparative (2022) + trend rows cite that same report.
 * Set SKIP_EMBED=1 to load figures only (no Bedrock). Uses the OWNER connection.
 *
 *   npm run seed:spike
 */
...
async function main() {
  if (!existsSync(seedPath)) throw new Error(`${seedPath} not found. Run: npm run convert`);
  ...
  try {
    ...
    for (const syn of seed.syndicates) {
      ...
      // One report per syndicate (the 2023 filing). Comparative + trend rows cite it.
      let primaryReportId: number | null = null;
      const reportYear = syn.years.find((y) => y.report);
      if (reportYear?.report) {
        primaryReportId = (
          await client.query<{ id: number }>(
            `INSERT INTO report (syndicate_number, year_of_account, title, source_url, n_pages)
             ...
             RETURNING id`,
            [syn.syndicate_number, reportYear.year_of_account, reportYear.report.title, reportYear.report.source_url, reportYear.report.n_pages]
          )
        ).rows[0].id;
      ...
      for (const y of syn.years) {
        await client.query(
          ...
          [
            ...
            primaryReportId, y.verified ?? false,
          ]
        );
      }
