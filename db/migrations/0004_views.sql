...
--   * YoY trend deltas  -> the hero "grew X while improving Y" queries.
...
-- Lower combined_ratio is better, so rank ascending; RoC higher is better.
...
SELECT
  ...
  percent_rank() OVER (PARTITION BY year_of_account ORDER BY return_on_capacity DESC NULLS LAST)    AS roc_pctile
FROM syndicate_year;

CREATE UNIQUE INDEX idx_mv_pct ON mv_peer_percentiles (syndicate_number, year_of_account);

-- Year-over-year deltas per syndicate (gwp growth, combined-ratio improvement).
...
SELECT
  ...
  gwp_gbp,
  combined_ratio,
  gwp_gbp - lag(gwp_gbp)   OVER w                       AS gwp_delta,
